...
const Farms: React.FC<FarmsProps> = (farmsProps) => {
  ...
  // console.log("debug->tokenMode", tokenMode, farmsLP)
  ...
  const pastaOnlyFarms = activeFarms.filter(
    (farm) => farm.lpSymbol.startsWith("JAVA"),
  ...
  console.log("debug Price", cakePrice, bnbPrice)
  ...
  const farmsList = useCallback(
    (farmsToDisplay, removed: boolean) => {
      ...
      const farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        console.log('debug->apy', apy.toNumber(), farm.pid, farm.lpTotalInQuoteToken, cakeRewardPerBlock, cakeRewardPerYear)
...
// console.log("debug-> farmlit", farmsList)
  ...
  return (
    <Page>
      <Heading as="h1" size="lg" color="primary" mb="50px" style={{ textAlign: 'center' }}>
        { 
          tokenMode ?
            TranslateString(10002, 'Stake tokens to earn JAVA')
            :
          TranslateString(320, 'Stake LP tokens to earn JAVA')
        }
      </Heading>
      <Heading as="h2" color="secondary" mb="50px" style={{ textAlign: 'center' }}>
        {TranslateString(10000, 'Deposit Fee will be used to buyback JAVA')}
      </Heading>
